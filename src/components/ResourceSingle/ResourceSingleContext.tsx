...
export const ResourceSingleProvider = (props: ResourceSingleProviderProps) => {
	...
	function getDefaultTsFolderShown() {
		...
		// find the folder
		const values = Object.values(props.tsFiles.trainingFiles.folders);
		const def = values.find((f) => f.slug === props.queryParams.download);
	...
	createEffect(() => {
		if (!import.meta.env.SSR) {
			...
			}=${encodeURIComponent(activeContent.name)}`;
			const currentRow =
				activeContent.rendered_contents?.htmlChapters[
					activeContent.activeRowIdx
				];

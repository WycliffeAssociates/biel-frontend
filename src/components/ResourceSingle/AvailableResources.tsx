...
export function AvailableResources(props: AvailableResourcesProps) {
	...
	return (
		<Show when={isBig()} fallback={<AvailableResourcesSmall {...props} />}>
			<div
				class={`hidden md:(flex shrink-0 flex-col gap-12 pie-4 pbe-8) ${
					props.classes || ""
				}`}
			>
				...
				<Show when={props.tsFiles?.trainingFiles}>
					<div>
						...
						<ul class="flex flex-col gap-4">
							<For
								each={tsFilesContainsSearch(
									menuSearchTerm,
									Object.entries(props.tsFiles!.trainingFiles!.folders),
								)}
							>
								{([key, value]) => (
									<TsFileDownload
...
function TsFileDownload(props: {
	...
	return (
		<li id={slugify(props.topLevelFolder)}>
			<button
				data-testid={slugify(props.topLevelFolder)}

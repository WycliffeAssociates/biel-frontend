...
export function ScripturalView() {
  ...
  function handleInternalTnLinks(e: MouseEvent) {
    ...
    if (activeRow === content.activeRowIdx) {
      ...
      if (hash) {
        const url = `${window.location.origin}${window.location.pathname}?resource-type=${content.resource_type}&book=${book}&chapter=${chapter}#${hash}`;
  ...
  async function fetchModalContent(url: string) {
    try {
      const response = await fetch(url, {
        headers: {
          [CustomXCacheTagHeader]: `${content.name}`,
        },
      });

    ...
    if (cmsData) {
      ...
      render(
        () => (
          <ConfirmationMessage
            dict={props.dict}
            didSucceed={false && formStatus().success}
...
function SubmitBtn(props: {text: string; disabled: boolean}) {
  return (
    <button
      ...
      type="submit"
      class={`px-14 bg-brand-base text-onSurface-invert border-2 border-solid border-[hsla(var(--clr-brand-100))] py-1  hover:(bg-brand-base/80 text-onSurface-invert) rounded-lg w-fit disabled:(opacity-50 cursor-not-allowed)`}
    >
      {props.text}
    </button>
  );

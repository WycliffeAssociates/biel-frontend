																...
																<ul class="flex flex-col gap-2 w-full">
																	<For each={menuLink.children?.non_featured}>
																		{(nonFeatured) => (
																			<li>
																				<a
																					class="w-full flex py-2 justify-between text-brand-base rounded-xl p-2"
...
function MobileNestedContainer(props: nestedNodeProps) {
	return (
		<div class="absolute top-0 left-0 bg-surface-primary z-10 h-screen overflow-auto w-full pie-4 pis-4">
			{props.children}
		</div>
	);
}

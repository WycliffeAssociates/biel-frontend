...
import type { languageType, Menu, MenuItem, WpPage } from "./customTypes/types";
...
function addDownloadAttrToBinaryTags(tags: Array<HTMLAnchorElement>) {
	// biome-ignore lint/suspicious/useIterableCallbackReturn: <no op. typical way to write>
...
export function determineShowGlobal({
	...
	page: WpPage;
	// biome-ignore lint/suspicious/noExplicitAny: <purposfeul unknown any>
...
export function nfdLowerNormalize(str: string) {
	return str
		.normalize("NFD")
		.toLowerCase()
		.replace(/\s+/g, "-")

...
async function handleInlineStyleInGeneratedFiles() {
  for await (const filepath of srcFiles) {
    ...
    const withoutInlinedStyleTag = text.replace(
...
async function purgeAgainstAdditionalGlobals() {
  ...
  // TODO - MAYBE  default extractor doesn't consider @, :, and /, so it isn't picking up stuff like md:block in server routes.  Just ignore this add'l globals route for now and only purge inline styles from wp
